...
function insert_cart_info2()
{
    ...
    $sql_detail = 'SELECT goods.goods_id, goods.goods_name, goods.shop_price, goods.goods_thumb' .
           ' FROM ' . $GLOBALS['ecs']->table('cart') . ' cart' . 
           " LEFT JOIN ".$GLOBALS['ecs']->table('goods')." goods ON cart.goods_id = goods.goods_id WHERE cart.session_id = '" . SESS_ID . "' AND cart.rec_type = '" . CART_GENERAL_GOODS . "'";
    $row = $GLOBALS['db']->GetRow($sql);
    $rows = $GLOBALS['db']->GetAll($sql_detail);
    ...
    $str = sprintf($GLOBALS['_LANG']['cart_info2'], $number);

	$detail = '';
	if(count($goods)){
		$detail = '<div style="display: none;" class="cart_pop">';
		foreach($goods as $good){
			$detail .= '
              	<dl>
                	<dt><a href="goods.php?id='.$good['goods_id'].'" target="_blank"><img src="'.$good['goods_thumb'].'" alt="'.$good['goods_name'].'" height="40" width="40"></a></dt>
                    <dd><a title="'.$good['goods_name'].'" href="goods.php?id='.$good['goods_id'].'" target="_blank">'.$good['goods_name'].'</a>  <span>×1</span></dd>
                   <dd>
                       <span class="price_cart">￥'.$good['shop_price'].'元</span>
                    <a class="del_cart" href="javascript:if (confirm(\'您确实要把该商品移出购物车吗？\')) location.href=\'flow.php?step=drop_goods&id='.$good['goods_id'].'\'; ">
                    [删除]</a><div class="clear"></div></dd>
                </dl>';
		}
		$detail	.= '
                            <p class="cart_count">共<font>'.$number.'</font>件商品<br>金额总计：<font>￥'.$amount.'元</font></p>
                <a class="add_cart" href="http://www.xuanlp.com/flow.php"> </a>
</div>
      </li>
      <li class="nav_showcat"></li>
      <li class="nav_check"><a class="" href="http://www.xuanlp.com/flow.php"> </a></li>
	  ';
	}

    return '<a href="flow.php" class="nav_carta" title="' . $GLOBALS['_LANG']['view_cart'] . '">' . $str . '</a>
	  '. $detail;
}
/**
 * 调用指定的广告位的广告
